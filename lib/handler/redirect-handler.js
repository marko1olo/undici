...
class RedirectHandler {
  ...
  constructor (dispatch, maxRedirections, opts, handler) {
    ...
    this.history = []
    // The origin the request is sent to. Client#request and Pool#request do not
    // pass one - the dispatcher fills it in from the url it was constructed with -
    // so it is read back from the controller once the request has started.
    this.currentOrigin = this.opts.origin ?? null
  }

  onRequestStart (controller, context) {
    this.currentOrigin = this.opts.origin ?? controller?.origin ?? null
  ...
  onResponseStart (controller, statusCode, headers, statusMessage) {
    ...
    const { origin, pathname, search } = util.parseURL(new URL(this.location, this.currentOrigin && new URL(this.opts.path, this.currentOrigin)))
    ...
    this.opts.headers = cleanRequestHeaders(this.opts.headers, removeContentHeaders, this.currentOrigin !== origin, this.stripHeadersOnRedirect, this.stripHeadersOnCrossOriginRedirect)
    this.opts.path = path
