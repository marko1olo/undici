...
class RedirectHandler {
  ...
  onResponseStart (controller, statusCode, headers, statusMessage) {
    ...
    const methodBeforeRedirect = this.opts.method
    ...
    // Check whether the redirect repeats the request we just made, which does not make
    // progress. This catches the case where Client/Pool try to handle cross-origin
    // redirects but fail and keep redirecting to the same URL in an infinite loop.
    // Two shapes are not loops: revisiting an earlier URL further back, such as the
    // session bounce /app -> /login -> /app, and a redirect to the same URL with a
    // different method, such as the 303 of Post/Redirect/Get. maxRedirections still
    // bounds the chain in both.
    const redirectUrlString = `${origin}${path}`
    if (this.opts.method === methodBeforeRedirect && this.history[this.history.length - 1]?.toString() === redirectUrlString) {
