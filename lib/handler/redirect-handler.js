...
class RedirectHandler {
  ...
  onResponseStart (controller, statusCode, headers, statusMessage) {
    ...
    // `Client#request` and `Pool#request` do not put an origin in the dispatch
    // options - the dispatcher derives it from the url it was constructed with -
    // so fall back to the origin the request was actually sent to. Without it
    // the first hop is never recorded, which both truncates the history handed
    // to the wrapped handler and delays the maxRedirections check above by a hop.
    const currentOrigin = this.opts.origin ?? controller?.origin
    if (currentOrigin) {
      this.history.push(new URL(this.opts.path, currentOrigin))
    }
