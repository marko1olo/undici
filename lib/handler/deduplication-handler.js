  ...
  #createWaitingHandler (handler) {
    ...
    waitingHandler.controller = {
      ...
      abort: (reason) => {
        ...
        const wasDone = waitingHandler.done
        ...
        // Mirror the real controller and surface the abort as onResponseError,
        // otherwise the waiting handler never settles. #errorWaitingHandler
        // marks the handler done before aborting and reports the error itself.
        if (!wasDone) {
          try {
            waitingHandler.handler.onResponseError?.(waitingHandler.controller, reason || new RequestAbortedError())
          } catch {
            // Ignore errors from waiting handlers
          }
        }
