  ...
  onResponseStart (
    ...
    if (statusCode === 304) {
      const handle304 = (cachedValue) => {
        ...
        // An ArrayBuffer view is the whole body, not a sequence of chunks -
        //  Buffer's inherited values() iterates it byte by byte.
        const cachedBody = ArrayBuffer.isView(cachedValue.body)
          ? [cachedValue.body]
          : cachedValue.body

        if (typeof cachedBody.values === 'function') {
          const bodyIterator = cachedBody.values()
          ...
          streamCachedBody()
        } else if (typeof cachedBody.on === 'function') {
