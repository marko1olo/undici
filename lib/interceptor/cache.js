...
function handleUncachedResponse (
  ...
  if (reqCacheControl?.['only-if-cached']) {
    return sendGatewayTimeout(handler)
  }
...
function handleResult (
  ...
  if (stale || revalidate) {
    // only-if-cached must not reach the origin, so the stored response is
    //  either reused as-is or we synthesize a 504
    ...
    if (reqCacheControl?.['only-if-cached']) {
      ...
      if (revalidate || staleResponseRequiresRevalidation(result, globalOpts.type)) {
      ...
      return sendCachedValue(handler, opts, result, age, null, true)
    }
