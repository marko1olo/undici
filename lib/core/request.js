...
class RequestController {
  ...
  // The origin the request is actually sent to. Client and Pool derive it from
  // the url they were constructed with rather than from the dispatch options,
  // so it is not otherwise observable by a handler.
  origin
  rawHeaders = null
  rawTrailers = null

  constructor (abort, origin) {
    this.#abort = abort
    this.origin = origin
  }
...
class Request {
  ...
  onRequestStart (abort, context) {
    ...
    this[kController] = new RequestController(abort, this.origin)
